...
const TEXT_SOURCE_KINDS = ["text_gdiplus_v3", "text_gdiplus_v2", "text_gdiplus", "text_ft2_source_v2", "text_ft2_source"] as const;
...
export class ObsClient {
  ...
  async getSourceSettingsForStudio(
    ...
    const matchingItem = await this.getStudioSceneItemById(sceneName, sceneItemId);
    if (!matchingItem) {
      throw new Error(`Scene item ${sceneItemId} was not found in scene '${sceneName}'.`);
    }
    ...
    try {
      inputInfo = await this.obs.call("GetInputSettings", { inputName: sourceName }) as ObsGetInputSettingsResponse;
    } catch (error) {
      throw new Error(
        `Source '${sourceName}' is not an OBS input or its settings could not be read: ${this.formatError(error)}`,
      );
    }
  ...
  private async getStudioSceneItemById(
    sceneName: string,
    sceneItemId: number,
    ...
    const inputKind = rawInputKind.length > 0 ? rawInputKind : await this.getInputKindSafe(sourceName);
